...
| **Gemini (lectura de cheques)** | Opcional, para `/lote`. `GEMINI_API_KEY` (y `GEMINI_MODEL`, por defecto `gemini-3.6-flash`). Sin la clave, las planillas y los PDF con texto se leen igual —en el servidor, sin salir— y las fotos se cargan a mano. **Falta la aprobación explícita del cliente** para mandar documentación de sus clientes a un servicio externo. |
