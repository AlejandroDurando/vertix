...
async function send(subject: string, html: string): Promise<EmailResult> {
...
export async function emailPrecalificacion(data: Record<string, unknown>): Promise<EmailResult> {
  ...
  const rows = Object.entries(data)
    .filter(([k]) => !["archivo"].includes(k))
    .map(([k, v], i) =>
      `<tr ${i % 2 === 0 ? "" : 'style="background:#f9f9f9"'}>
        <td style="padding:8px;color:#666;width:180px">${k}</td>
        <td style="padding:8px;font-weight:${k === "servicio" ? "600" : "normal"}">${String(v ?? "—")}</td>
      </tr>`
    )
    .join("");

  const archivoLine = data.archivo
    ? `<p style="margin-top:16px;color:#666;font-size:13px">📎 Archivo adjunto recibido — se enviará con el CRM.</p>`
    : "";
